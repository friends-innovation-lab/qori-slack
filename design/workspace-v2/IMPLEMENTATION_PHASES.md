...
## CC-1: Design-system foundation
**Prerequisites:** DDR-04 and DDR-07 answered. Record `git rev-parse origin/dev` in the PR. If it isn't `7f0366d8…`, run the diff in SPEC §0 and note any `workspace-projection.ts` change.
**Allowed:** `frontend/src/styles/tokens.css`, `typography.css` (only for `@font-face` if self-hosting), `reset.css`, `frontend/index.html`, `frontend/public/fonts/**` (if self-hosting).
**Must not change:** any `.tsx`, any `*.module.css`, `brief-document.css`.
**Tasks**
1. Replace `tokens.css` with `reference/tokens.reference.css` (drop the reference header comment; keep the `[KEEP]/[CHANGE]` annotations or strip them, but be consistent).
2. Swap the font `<link>` (TOKENS §4) or add `@font-face` rules.
3. `reset.css`: `a` color `--color-link`, hover `--color-link-hover`, `text-decoration-color: var(--color-border-emphasis)` (hover `currentColor`). `.skip-link` `z-index: var(--z-skip)`.
**Expected visual:** the whole app shifts to Instrument Sans, warm ink, brass focus rings and brass links. Brief/Plan prose now actually renders in Source Serif 4 (PF-02 aliases resolve). Layout is unchanged.
**Unchanged behavior:** everything.
**New tests:** `styles/__tests__/tokens.test.ts`, which parses `tokens.css` and asserts that every `var(--x)` used in `src/**/*.css` is defined (guards against PF-02 regressions).
**Acceptance:** zero undefined custom properties across `src/**/*.css`. Axe passes on Home, NewProject (existing a11y tests).
**Manual QA:** load Home, Projects, a Brief and a Plan. Check that fonts load (DevTools → Network), that focus rings are brass-deep on Tab, and that there's no FOIT longer than 100ms (`display=swap`).
...
**Tasks**
1. `AppShell`: add `WORKSPACE_ROUTE_PATTERNS = [] as const` and the `workspace` variant (COMPONENT_MAPPING §3.1). The array stays **empty** in this PR.
...
**New tests:** AppShell renders the workspace variant for a matched pattern (inject via prop in the test) · SideNav inverse keeps accessible names for all items · LifecycleRail inverse: locked node is `aria-disabled` and still focusable, current node has `aria-current="step"` · WorkspaceLayout drawer: Escape closes and focus returns to the toggle · ContextRail: arrow keys move tabs, Escape closes the overlay, close returns focus, the sheet traps focus.
