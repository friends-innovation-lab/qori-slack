...
**Drop-in source:** `reference/tokens.reference.css` is the complete proposed `tokens.css`. This document explains it; the CSS file wins if the two ever disagree.

## 1. Migration rules

1. **No token is renamed or deleted.** Every name in production `tokens.css` survives, so no existing CSS Module breaks. Values change where Workspace v2 requires it.
2. **Eight undefined tokens become aliases.** `document.module.css` already references them, and today they resolve to nothing (PF-02): `--font-serif`, `--font-sans`, `--color-text-secondary`, `--color-bg`, `--color-success-ink`, `--color-success-dark`, `--color-info-ink`, `--color-border-secondary`. After CC-1 the `var(..., #fallback)` hex fallbacks in `document.module.css` are dead and get removed in CC-2.
...
4. **Scope is global** (DDR-04). The token values apply app-wide: Home, Projects and forms also shift to Instrument Sans, brass and the new ink. Workspace-only *layout* (dark rails, context rail) is scoped to workspace routes.
...
| reduced-motion overrides | 0/100/100 | 0/0/0 | drawers must not slide |
...
Replace the Public Sans `<link>` in `frontend/index.html` with:
...
Weights loaded: Instrument Sans 400/500/600 · Source Serif 4 400/600 + italic 400 · Cormorant Garamond 500 · JetBrains Mono 500/600. **Weight 700 is not loaded.** Any `font-weight: 700` in workspace CSS maps to 600 (production `--text-h*-weight: 700` remains for non-workspace headings and renders as synthetic bold, which is acceptable until those pages migrate).
