...
- Non-workspace routes (Home, Projects, forms, StudyOverview) keep the current AppShell layout **and** the current token values (DDR-04 = B). AppShell sets `data-qori-surface="workspace"` on `<html>` only for workspace routes.
...
| DDR-04 | Token rollout scope | **DECIDED 2026-09-23: B, workspace-scoped first.** V2 values apply only under `:root[data-qori-surface="workspace"]` (TOKENS §1.4). Non-workspace pages are unchanged through UX-3A. App-wide promotion is a future decision | — |
